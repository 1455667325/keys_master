...
import { Form, Button, Input, TreeSelect, Select, Icon, DatePicker } from 'antd'
import { subNodeQuery, isEmpty } from '@/utils/common'
import { chunk, intersection, debounce } from 'lodash'
...
class Search extends Component {
        ...
        if (!err) {
          ...
          if (_fieldList.length) {
            _fieldList.forEach(e => {
              params[e.key] = values[e.key]
                ? subNodeQuery(e.data)(
                  Array.isArray(values[e.key]) ? values[e.key] : [values[e.key]]
                )
                : values[e.key]
            })
          }
   ...
   */
  yearOnChange = (v, key) => {
    this.props.form.setFieldsValue({ [key]: v })
  };
  renderComponent = (el = {}, idx) => {
    ...
    const formItemClass = 'search-from-item '
    // 自带组件时
    if (el.component) {
    ...
    switch (type) {
      ...
      case 'select': {
        ...
        let _data = data
        // 单选择框初始值
        let temp = !isEmpty(initValueSelect) ? { initialValue: initValueSelect } : {}
        let initialValue = { ...temp }
        // let initialValue = {};
        const v = null
        // 多选没有全部选项
        // if (!multiple) {
          ...
          >
            {getFieldDecorator(key, { ...initialValue })(
              ...
              >
                {_data.map((e, i) => {
                  return (
                    <Option key={i} value={e[value || 'value']} label={e[name || 'name']}>
                      {e[name || 'name']}
                    </Option>
                  )
                })}
          ...
          >
            {getFieldDecorator(key)(
              <TreeSelect
                ...
                className={className}
              >
                {this.renderTreeList(data, { name, value }, rootNode)}
              </TreeSelect>
            )}
          </Item>
        )
          ...
          >
            {getFieldDecorator(key)(
              <DateRange
                ...
                style={{ width: 638, ...style }}
                resetKey={this.randomKey}
                className="filter-form-item"
                {...other}
      ...
      case 'datePicker': {
        component = (
          ...
          >
            {getFieldDecorator(key)(
              <DatePicker
                className="long-time-form-item"
                mode={picker} //按年/月选择
                format={format} //按年/月选择
                placeholder={placeholder}
                onPanelChange={v => this.yearOnChange(v, key)}
              />
            )}
          </Item>
        )
   ...
   * 重置数据
   */
  handleReset = debounce((keys = []) => {
    const { fieldList = [], defaultFields, form, onReset } = this.props
    const fieldKeys = fieldList
      .concat(defaultFields)
      .filter(e => (e.check ? e.check(e) : true))
      .map(e => e.key)
   ...
   * @param data
   * @param config
   * @param rootNode 根节点是否可选
   * @returns {*}
   */
  renderTreeNode = (data = {}, config = {}, rootNode) => {
    return (
      <TreeNode
        value={data[config.value || 'stringId']}
        disabled={rootNode}
        data={data}
        title={data[config.name || 'name']}
        key={`${data[config.value || 'value']}`}
      >
        {data.childrenNode && data.childrenNode.length
          ? data.childrenNode.map(item => {
            return this.renderTreeNode(item, config)
          })
          : null}
      </TreeNode>
    )
   ...
   * @returns {*[]|*}
   */
  renderTreeList = (data = [], config, rootNode) => {
    if (Array.isArray(data)) {
      return data.map(el => {
        return this.renderTreeNode(el, config, rootNode)
      })
    } else {
      return this.renderTreeNode(data, config, rootNode)
    }
  };
  /**
   * 抽离指定的fileds
  ...
  render () {
    ...
    this.onChange()
    const isMarginLeft = this.setBtnsMarginleft(defaultFields, column)
      ? ''
      : ' search-from-no-margin-left '
    return (
      <div className="custom-form">
        <Form className="filter-form" layout="inline" onSubmit={this.handleSubmit}>
          <div className="default-filter-item">
            ...
            {/* 添加其他按钮 */}
            {this.props.children ? (
              <Item className={'search-item search-more-item'}>{this.props.children}</Item>
            ) : (
              ''
            )}
          </div>
